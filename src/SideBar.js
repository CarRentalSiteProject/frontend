...
//             // Call the onSearch function passed from Search.js
//             onSearch({
//                 chplace,
//                 chdate,
//                 redate
...
function SideBar({ startPlace, startDate, endDate, onSearch }) {
    const [chPlace, setChPlace] = useState(startPlace || ''); 
    const [chdate, setChDate] = useState(startDate || '');
    const [redate, setReDate] = useState(endDate || '');
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        onSearch(chPlace, chdate, redate); // Trigger manual search
    };
